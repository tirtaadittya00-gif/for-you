...
// Edit Icon Memory Game di Sini
    ...
    function bukaEnvelope() {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          // Munculkan animasi teks Happy Mother's Day
...
/* AUTO SCROLL BIAR TEKS GAK KEHENTI */
function smoothScrollMessage() {
  const el = document.getElementById("containerPesan");
  if (el) {
    el.scrollTo({
      top: el.scrollHeight,
      behavior: "smooth"
...
/* ENDING ZOOM HALUS */
setTimeout(() => {
  const last = document.getElementById("teksLucu");
  if (last) {
    last.style.transform = "scale(1.02)";
    last.style.transition = "2s ease";
  }
}, 7000);
